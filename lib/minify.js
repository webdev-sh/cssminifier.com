...
const blockTimeInSecs     = env.isProd ? oneHourInSecs : oneMinInSecs
const cacheFileTimeInSecs = env.isProd ? halfDayInSecs : oneMinInSecs
    ...
    md5File(orgFilename, (err, hash) => {
      ...
      client.get(hashKey, (err, result) => {
        ...
        client.set(fileKey, rid, 'EX', blockTimeInSecs, 'NX', (err, ok) => {
          ...
          const minified = new CleanCSS({ inline: 'none' }).minify(css)
          minified.css = minified.styles ? minified.styles : minified.errors.concat(minified.warnings).map(msg => `/* ${msg} */`).join('\n')

          fs.writeFile(minFilename, minified.css, err => {
