    ...
    md5File(orgFilename, (err, hash) => {
      ...
      client.get(hashKey, (err, result) => {
        ...
        client.set(fileKey, rid, 'EX', blockTimeInSecs, 'NX', (err, ok) => {
          ...
          const output = new CleanCSS({ inline: 'none' }).minify(css)
          const minCss = output.styles ? output.styles : output.errors.concat(output.warnings).map(msg => `/* ${msg} */`).join('\n')

          fs.writeFile(minFilename, minCss, err => {
